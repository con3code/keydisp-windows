...
1. [Releases](../../releases) から最新の `KeyDisp-vX.Y.Z-win-x64.zip` をダウンロードして展開します
2. `KeyDisp.exe` を任意の場所に置いて実行します (.NET のインストールは不要です)

現在のプレリリース版は**コード署名していません**。初回実行時に SmartScreen の警告が出た場合は
「詳細情報」→「実行」を選んでください。動作テストへのご協力・不具合報告は
[Issues](../../issues) へお願いします。
